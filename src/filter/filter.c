...
void filter_data(volatile axisData_t *gyroRateData, volatile axisData_t *gyroAccData, float gyroTempData, filteredData_t *filteredData)
{
	...
	errorMultiplierX = CONSTRAIN(errorMultiplierX * ABS(1.0f - ((setPoint.x + 100.0f) / (filteredData->rateData.x + 100.0f))) + 1.0f, 1.0f, 10.0f);
	errorMultiplierY = CONSTRAIN(errorMultiplierY * ABS(1.0f - ((setPoint.y + 100.0f) / (filteredData->rateData.y + 100.0f))) + 1.0f, 1.0f, 10.0f);
	errorMultiplierZ = CONSTRAIN(errorMultiplierZ * ABS(1.0f - ((setPoint.z + 100.0f) / (filteredData->rateData.z + 100.0f))) + 1.0f, 1.0f, 10.0f);
	...
	if (setPointNew)
	{
		...
		{
			filterConfig.roll_lpf_hz = CONSTRAIN(((float)filterConfig.i_roll_lpf_hz - 40.0f) * ABS(1.0f - ((setPoint.x * errorMultiplierX) / filteredData->rateData.x)), 10.0f, 500.0f);
			filter_biquad_init(filterConfig.roll_lpf_hz, &(lpfFilterStateRate.x));
		}
		if (setPoint.y != 0.0f && oldSetPoint.y != setPoint.y)
		{
			filterConfig.pitch_lpf_hz = CONSTRAIN(((float)filterConfig.i_pitch_lpf_hz - 40.0f) * ABS(1.0f - ((setPoint.y * errorMultiplierY) / filteredData->rateData.y)), 10.0f, 500.0f);
			filter_biquad_init(filterConfig.pitch_lpf_hz, &(lpfFilterStateRate.y));
		}
		if (setPoint.z != 0.0f && oldSetPoint.z != setPoint.z)
		{
			filterConfig.yaw_lpf_hz = CONSTRAIN(((float)filterConfig.i_yaw_lpf_hz - 40.0f) * ABS(1.0f - ((setPoint.z * errorMultiplierZ) / filteredData->rateData.z)), 10.0f, 500.0f);
