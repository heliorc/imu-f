...
void gpio_init(void)
{
	...
	if(GYRO_CS_TYPE)
	{
		hal_gpio_init_pin(GYRO_CS_PORT, GYRO_CS_PIN, GYRO_CS_MODE, GPIO_PULLUP, GYRO_CS_ALTERNATE);
	...
	{
		hal_gpio_init_pin(BOARD_COMM_CS_PORT, BOARD_COMM_CS_PIN, BOARD_COMM_CS_MODE, GPIO_PULLUP, BOARD_COMM_CS_ALTERNATE);
		HAL_GPIO_WritePin(BOARD_COMM_CS_PORT, BOARD_COMM_CS_PIN, GPIO_PIN_SET); //pin needs to default to high
	}
	hal_gpio_init_pin(BOARD_COMM_SCK_PORT,  BOARD_COMM_SCK_PIN,  GPIO_MODE_AF_PP, GPIO_NOPULL, BOARD_COMM_SCK_ALTERNATE);
	hal_gpio_init_pin(BOARD_COMM_MISO_PORT, BOARD_COMM_MISO_PIN, GPIO_MODE_AF_PP, GPIO_NOPULL, BOARD_COMM_MISO_ALTERNATE);
	hal_gpio_init_pin(BOARD_COMM_MOSI_PORT, BOARD_COMM_MOSI_PIN, GPIO_MODE_AF_PP, GPIO_NOPULL, BOARD_COMM_MOSI_ALTERNATE);
	...
	hal_gpio_init_pin(BOOTLOADER_CHECK_PORT, BOOTLOADER_CHECK_PIN, GPIO_MODE_OUTPUT_PP, GPIO_PULLUP, 0); 
