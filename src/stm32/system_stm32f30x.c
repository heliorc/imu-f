...
/******************************************************************************/
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
    ...
    FLASH->ACR = FLASH_ACR_PRFTBE | TARGET_FLASH_LATENCY;
 
    /* HCLK = SYSCLK / 1 */
    RCC->CFGR |= TARGET_AHB_DIV;

    /* PCLK2 = HCLK / 1 */
    RCC->CFGR |= TARGET_APBH2_DIV;

    /* PCLK1 = HCLK / 2 */
    RCC->CFGR |= TARGET_APBH1_DIV;

    /* PLL configuration */
    RCC->CFGR &= (uint32_t)((uint32_t)~(RCC_CFGR_PLLSRC | RCC_CFGR_PLLXTPRE | RCC_CFGR_PLLMULL));
    RCC->CFGR |= (uint32_t)(RCC_CFGR_PLLSRC_PREDIV1 | RCC_CFGR_PLLXTPRE_PREDIV1 | TARGET_PLL_MUL);
